...
wellSampler = function(nWells=1, wellDat=NULL, seismicDat, truthDat=NULL, modelFitter=NULL, 
  ...
  distMat = NULL
  prevFit = NULL
  nbatch = ceiling((nWells-minN)/batchSize) + 1
  for(i in 1:nbatch) {
    # startBatchT = proc.time()[3]
    # print(paste0("sampling batch ", i, "/", nbatch))
    
    if(i == 1) {
      # the first batch is of size minN
      thisBatchSize = min(c(minN, nWells))
      
    } else if(i == nbatch) {
      # the last batch size is leftover wells. Must account for minN batch and nbatch-2 regular batches
      thisBatchSize = nWells - minN - (nbatch-2)*batchSize
    ...
    if(is.null(preds)) {
      ...
      # already precomputed before for loop
    ...
    for(j in 1:thisBatchSize) {
      ...
      # update selection probabilities to account for repulsion effects
      thisLogitProbs = logitProbs - repelEffectsGrid
      ...
      if(!is.null(truthDat)) {
        ...
        thisWell = cbind(nextPts, nextVolFrac)
        colnames(thisWell) = c("east", "north", "volFrac")
      } else {
        thisWell = nextPts
        colnames(thisWell) = c("east", "north")
      }
